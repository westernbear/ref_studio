...
const pythonPath = resolve(workspace, ".rvs-cache/python-3.12.14/bin/python3.12")
...
async function download(artifact) {
  if (artifact.digest !== undefined) return { ...artifact, localPath: null, verified: "oci-digest" }
  const integrity = artifact.sha256 === undefined ? artifact.integrity?.match(/^(sha512)-(.+)$/) : undefined
  const algorithm = artifact.sha256 === undefined ? integrity?.[1] : "sha256"
  const expected = artifact.sha256 ?? (integrity?.[2] === undefined ? undefined : Buffer.from(integrity[2], "base64").toString("hex"))
  if (algorithm === undefined || expected === undefined) {
    throw new Error(`SUPPLY_PIN_UNAVAILABLE ${artifact.name} has no supported integrity`)
  }
  ...
  try {
    ...
    if (createHash(algorithm).update(bytes).digest("hex") !== expected) throw new Error(`cached digest mismatch ${artifact.name}`)
    return { ...artifact, localPath: target.slice(workspace.length + 1), verified: algorithm }
  } catch (error) {
  ...
  if (!response.ok) {
    throw new Error(`SUPPLY_PIN_UNAVAILABLE ${artifact.name} HTTP ${response.status} ${artifact.url}`)
  }
  const bytes = Buffer.from(await response.arrayBuffer())
  const actual = createHash(algorithm).update(bytes).digest("hex")
  if (actual !== expected) {
    throw new Error(`SUPPLY_PIN_UNAVAILABLE ${artifact.name} ${algorithm}=${actual} expected=${expected}`)
  }
...
async function fetchMetadata(url) {
  ...
  if (!response.ok) throw new Error(`SUPPLY_PIN_UNAVAILABLE license metadata HTTP ${response.status} ${url}`)
  return response.json()
}

async function npmRegistryLicense(entry) {
  const metadata = await fetchMetadata(`https://registry.npmjs.org/${entry.name.replace("/", "%2f")}/${entry.version}`)
  const direct = licenseText(metadata.license)
  if (direct !== undefined) return direct
  if (Array.isArray(metadata.licenses)) {
    const values = metadata.licenses.map(licenseText).filter((value) => value !== undefined)
    if (values.length > 0) return values.join(" OR ")
  }
  throw new Error(`SUPPLY_PIN_UNAVAILABLE npm license missing ${entry.name}@${entry.version}`)
}
...
async function pythonRegistryLicense(entry) {
  ...
  const metadata = await fetchMetadata(`https://pypi.org/pypi/${encodeURIComponent(entry.name)}/${encodeURIComponent(registryVersion)}/json`)
  ...
  const classifiers = Array.isArray(metadata.info?.classifiers)
    ? metadata.info.classifiers.filter((value) => value.startsWith("License :: "))
    : []
  if (classifiers.length > 0) return classifiers.join("; ")
  const licenseFiles = Array.isArray(metadata.info?.license_files)
    ? metadata.info.license_files.filter((value) => typeof value === "string" && value !== "")
    : []
  if (licenseFiles.length > 0) return `embedded:${licenseFiles.join(",")}`
  throw new Error(`SUPPLY_PIN_UNAVAILABLE Python license missing ${entry.name}@${entry.version}`)
}
...
const hydrated = []
for (const artifact of supply.artifacts) hydrated.push(await download(artifact))
...
const npmManifestPath = resolve(workspace, "runtime/npm-artifact-manifest.json")
const npmManifest = JSON.parse(await readFile(npmManifestPath, "utf8"))
const npmLicenses = packageLicenses(JSON.parse(run("pnpm", ["licenses", "list", "--json", "--long"])))
npmManifest.packages = await mapConcurrent(npmManifest.packages, 12, async (entry) => ({
  ...entry,
  license: npmLicenses.get(`${entry.name}@${entry.version}`) ?? (await npmRegistryLicense(entry)),
}))
await writeFile(npmManifestPath, `${JSON.stringify(npmManifest, null, 2)}\n`)

const pythonManifestPath = resolve(workspace, "runtime/python-wheel-manifest.json")
...
const pythonLicenses = JSON.parse(run(venvPythonPath, ["-c", pythonLicenseScript]))
pythonManifest.packages = await mapConcurrent(pythonManifest.packages, 12, async (entry) => {
  const localLicense = pythonLicenses[`${entry.name.toLowerCase()}@${entry.version}`]
  return {
    ...entry,
    license: localLicense === undefined || localLicense === "UNKNOWN" ? await pythonRegistryLicense(entry) : localLicense,
  }
})
await writeFile(pythonManifestPath, `${JSON.stringify(pythonManifest, null, 2)}\n`)
...
for (const name of ["chrome-for-testing", "wanted-sans", "inter"]) {
  ...
  run("unzip", ["-q", "-o", resolve(workspace, artifact.localPath), "-d", destination])
}

const fontRoot = resolve(workspace, "verification/contract/fonts")
await mkdir(fontRoot, { recursive: true })
const wantedFont = await findFile(resolve(assetRoot, "wanted-sans"), ["WantedSansVariable.ttf"])
const interFont = await findFile(resolve(assetRoot, "inter"), ["InterVariable.ttf"])
if (wantedFont === undefined || interFont === undefined) {
  throw new Error("SUPPLY_PIN_UNAVAILABLE required fixture fonts absent from pinned archives")
}
...
const debianBytes = await readFile(resolve(workspace, "runtime/debian-snapshot-manifest.json"), "utf8")
const containerManifestPath = resolve(workspace, "runtime/container-child-digest-manifest.json")
let builtImages = []
try {
  const existingContainers = JSON.parse(await readFile(containerManifestPath, "utf8"))
  if (Array.isArray(existingContainers.builtImages)) builtImages = existingContainers.builtImages
} catch (error) {
...
const ffmpegManifestPath = resolve(workspace, "runtime/ffmpeg-build-manifest.json")
...
try {
  existingFfmpegManifest = JSON.parse(await readFile(ffmpegManifestPath, "utf8"))
