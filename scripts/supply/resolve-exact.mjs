...
function parsePnpmPackages(lockText) {
  ...
  for (const line of lockText.split("\n")) {
    ...
    if (packageMatch?.[1] !== undefined) {
      ...
      if (separator <= 0)
        throw new Error(`SUPPLY_PIN_UNAVAILABLE invalid pnpm key ${key}`);
      current = {
    ...
    if (/^    (cpu|os|libc|engines):/.test(line))
      current.platformMarkers.push(line.trim());
  }
...
function parseUvPackages(lockText) {
  ...
  for (const block of lockText.split("[[package]]").slice(1)) {
    ...
    const artifacts = [
      ...block.matchAll(
        /\{ url = "([^"]+)", hash = "sha256:([a-f0-9]{64})"[^}]*\}/g,
      ),
    ].map((match) => ({ url: match[1], sha256: match[2] }));
...
if (
  run("pnpm", ["--version"]) !== "11.20.0" ||
  !run("uv", ["--version"]).startsWith("uv 0.11.8 ")
) {
  throw new Error(
    "SUPPLY_PIN_UNAVAILABLE pinned pnpm 11.20.0 and uv 0.11.8 are required",
  );
}
if (!existsSync(pythonPath)) {
  const pythonImage = supply.artifacts.find(
    (artifact) => artifact.name === "python-image",
  );
  if (pythonImage?.digest === undefined) {
    throw new Error(
      "SUPPLY_PIN_UNAVAILABLE pinned Python image digest is missing",
    );
  }
