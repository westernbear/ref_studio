...
function field(file, name) {
  return execFileSync("dpkg-deb", ["--field", file, name], {
    encoding: "utf8",
  }).trim();
}
  ...
  try {
    ...
    if (
      ...
      manifest.packages.every(
        (entry) => entry.url !== null && entry.aptDigest !== null,
      )
    ) {
      process.stdout.write(
        `${JSON.stringify({ status: "debian-closure-reused", packages: manifest.packages.length })}\n`,
      );
      process.exit(0);
    }
  } catch (error) {
...
execFileSync(
  "docker",
  ["run", "--rm", "-v", `${output}:/out`, nodeImage, "bash", "-c", shell],
  {
    cwd: workspace,
    stdio: "inherit",
  },
...
const urls = new Map(
  [
    ...uriText.matchAll(
      /^'([^']+\.deb)'\s+([^\s]+\.deb)\s+\d+(?:\s+((?:SHA256|SHA512|MD5Sum):[a-f0-9]+))?\s*$/gm,
    ),
  ].map((match) => [
    match[2],
    { url: match[1], aptDigest: match[3] ?? "not-provided" },
  ]),
);
const packages = [];
...
if (packages.some((entry) => entry.url === null || entry.aptDigest === null)) {
  throw new Error(
    "SUPPLY_PIN_UNAVAILABLE Debian package URL or APT digest missing",
  );
}
await writeFile(
  manifestPath,
...
await writeFile(
  resolve(workspace, "runtime/debian-packages.lock"),
  `${packages.map((entry) => `${entry.name}=${entry.version}`).join("\n")}\n`,
);
process.stdout.write(
  `${JSON.stringify({ status: "debian-closure-resolved", packages: packages.length, snapshotDigest })}\n`,
);
