...
const expectedChrome = process.env.EXPECTED_CHROMIUM_VERSION ?? "151.0.7922.138"
...
function requireMatch(label, actual, expected) {
  if (!actual.includes(expected)) {
    throw new Error(`RUNTIME_VERSION_MISMATCH ${label} expected=${expected} actual=${JSON.stringify(actual)}`)
  }
}
...
async function requireNetworkBlocked() {
  ...
  if (!dnsBlocked || !ipBlocked) {
    throw new Error(`RUNTIME_NETWORK_NOT_ISOLATED dnsBlocked=${dnsBlocked} ipBlocked=${ipBlocked}`)
  }
