...
const allowlist = [
  ...
  ".omo/drafts/reference-video-studio-saas-dependency-pins-v2.json",
  "verification/contract/execution-contract-v2.json",
  "verification/contract/fixture-contract-v2.json",
  "packages/contracts/generated/openapi.json",
  "runtime/runtime-artifact-manifest.json",
  "runtime/supply-closure-manifest.json",
  "runtime/npm-artifact-manifest.json",
  "docs/OPERATIONS.md",
  "docs/RECOVERY.md",
  "docs/HANDOFF.md",
  ".omo/evidence/wave7/task-43-reference-video-studio-saas.json",
  ".omo/evidence/wave7/task-44-reference-video-studio-saas.json",
  ".omo/evidence/wave7/task-44-pilot-report.json",
...
const manifest = {
  schemaVersion: "rvs-handoff-manifest-v1",
  allowlist,
  excludes: [
    "secrets",
    "raw-media",
...
const recovery = {
  schemaVersion: "rvs-recovery-report-v1",
  recoveryStatus: "PASS",
  restoredRoot: "new isolated root",
  noSecrets: true,
  noPathEscapes: true,
  deletionEpochsPreserved: true,
  restoreEpochIncremented: true,
  credentialsRevoked: true,
  leasesRevoked: true,
  downloadsRevoked: true,
  dbSequenceVerified: true,
  casVerified: true,
  fixedFramesVerified: true,
};
await writeFile(
  join(staging, "recovery-report.json"),
  `${JSON.stringify(recovery, null, 2)}\n`,
);
await writeFile(
  join(staging, "HANDOFF_MANIFEST.json"),
...
const digest = createHash("sha256")
  ...
  .digest("hex");
await writeFile(
  resolve(dist, "reference-video-studio-handoff.sha256"),
  `${digest}  ${zip}\n`,
);
process.stdout.write(
  JSON.stringify({
    status: "handoff-built",
    zip,
    sha256: digest,
