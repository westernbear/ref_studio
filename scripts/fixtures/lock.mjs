...
function run(command, args) {
  const containerArgs = args.map((argument) =>
    argument.startsWith(workspace) ? `/workspace${argument.slice(workspace.length)}` : argument,
  )
...
function rawFrameSha256(input, frames) {
  ...
  if (digest === undefined) throw new Error("fixture raw-frame hash was not emitted")
  return digest
}
...
function audioPcmSha256(filtergraph) {
  ...
  const digest = output.match(/SHA256=([a-f0-9]{64})/)?.[1]
  if (digest === undefined) throw new Error("fixture audio hash was not emitted")
  return digest
}
...
async function generateMedia(contract, fixture, filtergraph, root) {
  ...
  run(ffmpeg, ["-hide_banner", "-nostdin", "-i", filterSource, ...contract.generator.videoOnlyArgvSuffix, "-threads", "1", "-y", videoOnly])
  run(ffmpeg, [...contract.generator.audioArgvPrefix.slice(1), fixture.audioFiltergraph, ...contract.generator.audioArgvSuffix, "-y", audio])
  run(ffmpeg, [
    "-hide_banner",
    "-nostdin",
    "-i",
...
async function generatePass(contract, root, generatorClosureSha256) {
  ...
  for (const fixture of contract.fixtures) {
    ...
    const media = await generateMedia(contract, fixture, fixture.videoFiltergraph, fixtureRoot)
    const variants = {}
    for (const [name, filtergraph] of Object.entries(fixture.variantFiltergraphs ?? {})) {
      const variantMedia = await generateMedia(contract, fixture, filtergraph, resolve(fixtureRoot, "variants", name))
      variants[name] = {
        variantFiltergraphSha256: sha256(filtergraph),
        variantTruthSha256: sha256(JSON.stringify(fixture.truth.variants[name])),
    ...
    if (Object.keys(variants).length > 0) {
      await writeFile(resolve(fixtureRoot, "variants", "manifest.json"), `${JSON.stringify({ id: fixture.id, variants }, null, 2)}\n`)
    }
    entries.push({
      id: fixture.id,
      fps: fixture.fps,
    ...
    process.stdout.write(`${JSON.stringify({ status: "fixture-generated", id: fixture.id })}\n`)
  }
  return entries
}
...
const lockArgument = option("--lock") ?? "verification/contract/fixture-manifest.lock.json"
const noWriteLock = process.argv.includes("--no-write-lock") || process.argv.includes("--verify-lock")
const contractBytes = await readFile(resolve(workspace, contractArgument))
const contract = JSON.parse(contractBytes.toString("utf8"))
if (contract.schemaVersion !== "rvs-fixture-contract-v2") throw new Error("invalid fixture contract schema")
if (!Array.isArray(contract.fixtures) || contract.fixtures.length !== 16) throw new Error("FIXTURE_COUNT_MISMATCH expected 16 fixtures")
if (new Set(contract.fixtures.map((fixture) => fixture.id)).size !== 16) throw new Error("FIXTURE_ID_DUPLICATE")
...
if (
  ...
  throw new Error("SUPPLY_PIN_UNAVAILABLE pinned GPL FFmpeg 8.0.1 with x264 is required")
}
const closureParts = await Promise.all([
  ...
  readFile(resolve(workspace, "verification/contract/fonts/WantedSansVariable.ttf")),
...
const passTwoRoot = resolve(workspace, `.rvs-cache/fixture-pass-${process.pid}`)
const passOne = await generatePass(contract, passOneRoot, generatorClosureSha256)
const passTwo = await generatePass(contract, passTwoRoot, generatorClosureSha256)
const passOneSha256 = sha256(JSON.stringify(passOne))
const passTwoSha256 = sha256(JSON.stringify(passTwo))
if (passOneSha256 !== passTwoSha256) throw new Error("FIXTURE_NONDETERMINISTIC two generation passes differ")
...
if (noWriteLock) {
  ...
  if (JSON.stringify(existing) !== JSON.stringify(lock)) throw new Error("FIXTURE_LOCK_MISMATCH")
  process.stdout.write(`${JSON.stringify({ status: "fixtures-verified", fixtures: passOne.length, passSha256: passOneSha256, lock: lockArgument })}\n`)
} else {
  await writeFile(resolve(workspace, contract.fixtureLock.path), `${JSON.stringify(lock, null, 2)}\n`)
  process.stdout.write(`${JSON.stringify({ status: "locked", fixtures: passOne.length, passSha256: passOneSha256 })}\n`)
}
