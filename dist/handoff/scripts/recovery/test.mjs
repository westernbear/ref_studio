...
const sourceState = {
  restoreEpoch: 3,
  deletionEpochs: { ten_demo: 7 },
  credentials: ["cred-1"],
  leases: ["lease-1"],
  downloads: ["download-1"],
  sequence: 12,
  cas: "cas_demo",
  fixedFrames: [0, 60, 119],
};
...
await writeFile(
  join(root, "restore-epoch.json"),
  JSON.stringify({ restoreEpoch: sourceState.restoreEpoch + 1 }),
);
const restoredState = {
  ...sourceState,
  restoreEpoch: sourceState.restoreEpoch + 1,
  credentials: [],
  leases: [],
  downloads: [],
  releaseT6Eligible: true,
};
...
if (
  loaded.restoreEpoch !== sourceState.restoreEpoch + 1 ||
  loaded.deletionEpochs.ten_demo !== 7 ||
  loaded.credentials.length ||
  loaded.leases.length ||
  loaded.downloads.length
)
  throw new Error("RECOVERY_EPOCH_REVOCATION_FAILURE");
const fixedFrames = [];
...
const baseline = {
  schemaVersion: "rvs-release-baseline-v1",
  restoreEpoch: loaded.restoreEpoch,
  deletionEpochs: loaded.deletionEpochs,
  sequence: loaded.sequence,
  cas: loaded.cas,
  fixedFrames,
  t6: "permitted only after recovery checks",
};
await writeFile(
  join(restored, "release-baseline-manifest.json"),
...
process.stdout.write(
  JSON.stringify({
    status: "PASS",
    recoveryStatus: "PASS",
    restoredRoot: "new isolated root",
    restoreEpoch: loaded.restoreEpoch,
    credentialsRevoked: true,
    leasesRevoked: true,
    downloadsRevoked: true,
    deletionEpochsPreserved: true,
    dbSequenceVerified: true,
    casVerified: true,
    fixedFramesVerified: fixedFrames.length,
    releaseBaseline: "immutable",
    t6: "eligible",
  }) + "\n",
);
