...
Initial admin credentials can be supplied in the root `.env` before `pnpm --filter @rvs/api db:reset` or `pnpm --filter @rvs/api db:verify`: `RVS_INITIAL_ADMIN_EMAIL`, `RVS_INITIAL_ADMIN_PASSWORD`, and optional `RVS_INITIAL_ADMIN_NAME`.

Worker compose uses `RVS_API_BASE_URL`; from a Docker worker talking to the host-published web/API port, use `http://host.docker.internal:3100`.
