...
test.describe("shared sign-in", () => {
  test("validates, reveals, and safely clears a failed creator sign-in @sign-in", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: "Forgot Secret?" })).toHaveAttribute("href", "/support")
    ...
    await expect(page.getByRole("alert", { name: "Sign-in error" })).toHaveText("The identifier or secret could not be verified.")
    await expect(identifier).toHaveValue("creator@example.invalid")
    await expect(secret).toHaveValue("")
    await expect(secret).toBeFocused()
  })

  test("renders the admin variant with an internal return path @sign-in", async ({ page }) => {
