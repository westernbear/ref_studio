...
test("@scene-review enforces evidence, authority, and the T1-T5 predecessor chain", async ({
  page,
}) => {
  ...
  await expect(
    page.locator("[data-control-id^='scene_review_approval:']"),
  ).toHaveCount(18);
  await expect(
    page
      .locator("[data-control-id^='scene_review_approval:']")
      .evaluateAll((nodes) =>
    ...
    Array.from(
      { length: 18 },
      (_, index) => `scene_review_approval:${index + 1}`,
    ),
  );
  ...
  for (const gate of ["T2", "T3", "T4", "T5"]) {
    ...
    await expect(
      page.getByText(
        `${gate === "T5" ? 5 : Number(gate.slice(1))} append-only receipts`,
      ),
    ).toBeVisible();
  }
  await expect(
    page.getByRole("button", { name: "Launch final render" }),
  ).toBeEnabled();
});
