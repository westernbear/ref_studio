...
const manifest = JSON.parse(readFileSync(resolve(root, "tests/control-manifest.json"), "utf8"))
...
describe("shared control contract projection", () => {
  it("projects exactly 151 unique source controls", () => {
    ...
    expect(new Set(manifest.controls.map((control) => control.id)).size).toBe(151)
  })
  it("preserves executable control fields and projection metadata", () => {
    for (const control of manifest.controls) {
      for (const field of ["id", "action", "path", "state", "result"]) expect(control[field]).toBeTruthy()
  ...
  it("uses semantic disabled behavior and visible focus", () => {
    ...
    expect(readFileSync(resolve(root, "apps/web/src/styles/primitives.css"), "utf8")).toContain(":focus-visible")
  })
})
