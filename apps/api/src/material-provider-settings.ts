...
// required env var), same derivation as ai-provider-settings.ts but with a
// distinct salt: domain separation both from session introspection and from
// the LLM provider's own secret, so the two ciphertexts are never
// interchangeable even though they may share the same underlying secret key.
const deriveKey = (secretKey: string) =>
  scryptSync(secretKey, "rvs-material-provider-settings", 32);
