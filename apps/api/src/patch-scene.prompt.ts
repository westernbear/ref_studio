...
- A "kind": "text" element may carry "weight": one of "regular" (body lines), "bold" (emphasis, and what text renders at if you name no weight) or "black" (the heaviest the font goes -- for a number or a headline that has to land). No other value exists; never emit a number. Feedback like "make the number stand out" or "the headline is getting lost" is usually a weight edit on that one element, not a heavier setting on every element in the beat -- hierarchy comes from the contrast between them.
